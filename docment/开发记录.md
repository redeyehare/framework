...
http-->response_manager-->trigger_manager-->response_manager

event-->trigger_manager-->push_manager-->发送

trigger_manager-->trigger_table-->marker_table-->trigger_manager
trigger_table-->announcement
trigger_table-->是否有效
response_manager--isread-->marker_table
发送--isread-->marker_table
```
...
http--request-->response_manager-->response--request-->http
...
事件触发-->Trigger_Manager-->消息queue==>PushManager
消息queue-->时间戳排序,即时消息优先级更高
PushManager-->即时Instant-->每3分钟推送消息-->根据范围复制出用户列表
PushManager-->预定Scheduled-->每30分钟推送消息-->根据范围复制出用户列表--> 发送消息


Trigger_Manager-->trigger_table-->announcement表-->marker_table
